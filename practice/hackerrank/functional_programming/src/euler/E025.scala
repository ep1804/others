...
object NDigitFibonacciNumber {
  ...
  // (digits, index)
  lazy val fib2 = (0, 0) #:: (fib zip Stream.from(1)).map { case (a, b) => (a.toString.length, b) }

  // series of fibonacci indices where digit changes)
  lazy val fib3: Stream[Int] = (fib2 zip fib2.tail).filter { case ((a, _), (b, _)) => a != b }. map { case (a, b) => b._2 }
